...

{
    ...
    public class GridCoreServer<T> : IGridServer<T>
    {
        ...
        public GridCoreServer(IEnumerable<T> items, QueryDictionary<StringValues> query, bool renderOnlyRows,
            string gridName, Action<IGridColumnCollection<T>> columns = null, int? pageSize = null, int? totalRecord=null,
            ...
            IColumnBuilder<T> columnBuilder = null)
        {
            ...
            if (pageSize.HasValue)
                WithPaging(pageSize.Value,totalRecord.Value);
        }
            ...
            IColumnBuilder<T> columnBuilder = null)
        {
            ...
            _source = new SGridCore<T>(items, query, renderOnlyRows, pagerViewName, columnBuilder);
            _source.RenderOptions.GridName = gridName;
            _source.Pager.ItemsCount = totalRecord.Value;
            columns?.Invoke(_source.Columns);
            if (!string.IsNullOrWhiteSpace(language))
                _source.Language = language;
            if (pageSize.HasValue)
        ...
        public IGridServer<T> ChangeSkip(bool enable)
        {
            _source.Pager.ChangePageSkip = enable;
            return this;
        }
        ...
        public IGridServer<T> WithPaging(int pageSize,int totalRecord)
        {
            return WithPaging(pageSize, totalRecord,0);
        }
        public IGridServer<T> WithPaging(int pageSize, int totalRecord, int maxDisplayedItems)
        {
            return WithPaging(pageSize, totalRecord,maxDisplayedItems, string.Empty);
        }

        public IGridServer<T> WithPaging(int pageSize, int totalRecord,int maxDisplayedItems, string queryStringParameterName)
        {
            _source.EnablePaging = true;
            _source.Pager.PageSize = pageSize;
            _source.Pager.ItemsCount = totalRecord;
